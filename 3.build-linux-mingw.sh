...
for CUR_PLATFORM in ${TARGET_PLATFORMS}; do
    ...
    case "${CUR_PLATFORM}" in
	    ...
	    chmod +x build_detect_platform || exist_error "Failed to make LevelDB executable"
            ...
            PATH=${platform_src_dir}/qt/bin:$PATH ${platform_src_dir}/qt/bin/qmake -makefile -spec unsupported/win32-g++-cross MINIUPNPC_LIB_PATH=${platform_src_dir}/miniupnpc-1.9 MINIUPNPC_INCLUDE_PATH=${platform_src_dir} BDB_LIB_PATH=${platform_src_dir}/db-4.8.30.NC/build_unix BDB_INCLUDE_PATH=${platform_src_dir}/db-4.8.30.NC/build_unix BOOST_LIB_PATH=${platform_src_dir}/boost_1_55_0/stage/lib BOOST_INCLUDE_PATH=${platform_src_dir}/boost_1_55_0 BOOST_LIB_SUFFIX=-mt-s BOOST_THREAD_LIB_SUFFIX=_win32-mt-s OPENSSL_LIB_PATH=${platform_src_dir}/openssl-1.0.1g OPENSSL_INCLUDE_PATH=${platform_src_dir}/openssl-1.0.1g/include QRENCODE_LIB_PATH=${platform_src_dir}/qrencode-3.4.2/.libs QRENCODE_INCLUDE_PATH=${platform_src_dir}/qrencode-3.4.2 USE_UPNP=1 USE_QRCODE=1 INCLUDEPATH=${platform_src_dir} DEFINES=BOOST_THREAD_USE_LIB QMAKE_LRELEASE=lrelease USE_BUILD_INFO=1 BITCOIN_NEED_QT_PLUGINS=1 RELEASE=1 USE_LEVELDB=1 || exit_error "qmake failed"
	    ...
	    chmod +x ./build_detect_platform || exist_error "Failed to make LevelDB executable"
            ...
            PATH=${WORKSPACE}/dependencies/x86_64/qt/bin:$PATH ${WORKSPACE}/dependencies/x86_64/qt/bin/qmake -makefile -spec unsupported/win32-g++-cross MINIUPNPC_LIB_PATH=${WORKSPACE}/dependencies/x86_64/miniupnpc-1.9 MINIUPNPC_INCLUDE_PATH=${WORKSPACE}/dependencies/x86_64 BDB_LIB_PATH=${WORKSPACE}/dependencies/x86_64/db-4.8.30.NC/build_unix BDB_INCLUDE_PATH=${WORKSPACE}/dependencies/x86_64/db-4.8.30.NC/build_unix BOOST_LIB_PATH=${WORKSPACE}/dependencies/x86_64/boost_1_55_0/stage/lib BOOST_INCLUDE_PATH=${WORKSPACE}/dependencies/x86_64/boost_1_55_0 BOOST_LIB_SUFFIX=-mt-s BOOST_THREAD_LIB_SUFFIX=_win32-mt-s OPENSSL_LIB_PATH=${WORKSPACE}/dependencies/x86_64/openssl-1.0.1g OPENSSL_INCLUDE_PATH=${WORKSPACE}/dependencies/x86_64/openssl-1.0.1g/include QRENCODE_LIB_PATH=${WORKSPACE}/dependencies/x86_64/qrencode-3.4.2/.libs QRENCODE_INCLUDE_PATH=${WORKSPACE}/dependencies/x86_64/qrencode-3.4.2 USE_UPNP=1 USE_QRCODE=1 INCLUDEPATH=${WORKSPACE}/dependencies/x86_64 DEFINES=BOOST_THREAD_USE_LIB QMAKE_LRELEASE=lrelease USE_BUILD_INFO=1 BITCOIN_NEED_QT_PLUGINS=1 RELEASE=1 || exit_error "qmake failed"
